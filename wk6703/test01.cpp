...
int maxSurvival(int A, int B, area X, area Y, area Z,
	       	int last, std::map<std::pair<int, int>, int>& memo)
{
	if( A <= 0 || B<=0)
		return 0;
	std::pair<int, int> cur = std::make_pair(A, B);
	if(memo.find(cur)!=memo.end())
		return memo[cur];
	int temp;
	switch(last){
	case 1:
        	temp = 1 + std::max(maxSurvival(A + Y.a, B + Y.b,
                                   X, Y, Z, 2, memo),
                       maxSurvival(A + Z.a, B + Z.b,
                                  X, Y, Z, 3, memo));
        break;
    	case 2:
        	temp = 1 + std::max(maxSurvival(A + X.a, B + X.b,
                                  X, Y, Z, 1, memo),
                       maxSurvival(A + Z.a, B + Z.b,
                                  X, Y, Z, 3, memo));
        break;
...
int main(int argc, char *argv[]){
	area X(3, 2);
    	...
    	int A = 20;
    	int B = 8;
    	std::cout << getMaxSurvivalTime(A, B, X, Y, Z) << "\n";
	return 0;
}
